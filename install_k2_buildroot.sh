...
OBICO_REPO="https://github.com/frybin/moonraker-obico.git"
...
run_privileged() {
  if [ "$(id -u)" = "0" ]; then
    ...
    exit_on_error "Need root privileges to run: $*"
  fi
}
...
ensure_deps() {
  if is_k2_buildroot; then
    report_status "Checking K2 runtime dependencies..."
    for required_cmd in python3 pip3 ffmpeg systemctl; do
      if ! command -v "${required_cmd}" >/dev/null 2>&1; then
        exit_on_error "Missing required command: ${required_cmd}"
      fi
    done
    ensure_venv
  ...
  if ! command -v apt-get >/dev/null 2>&1; then
    exit_on_error "apt-get is not available on this system."
  fi
  ...
  run_privileged sed -i '/^deb http:\/\/apt.armbian.com buster main buster-utils buster-desktop/s/^/# /' /etc/apt/sources.list.d/armbian.list 2>/dev/null || true
  run_privileged apt-get --allow-releaseinfo-change -o Acquire::Check-Valid-Until=false -o Acquire::Check-Date=false update
  run_privileged apt-get install --yes ${PKGLIST}
  ensure_venv
  debug Running... "${OBICO_ENV}"/bin/pip3 install -q -r "${OBICO_DIR}"/requirements.txt
  "${OBICO_ENV}"/bin/pip3 install -q -r "${OBICO_DIR}"/requirements.txt
  echo ""
...
recreate_service() {
  ...
  report_status "Creating moonraker-obico systemctl service..."
  run_privileged /bin/sh -c "cat > /etc/systemd/system/${OBICO_SERVICE_NAME}.service" <<EOF
...
EOF
  ...
  run_privileged systemctl enable "${OBICO_SERVICE_NAME}"
